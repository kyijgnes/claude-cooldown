...
class Mascot {
    ...
    private companion object {
        ...
        // **박자 맞히기** — 바닥에 멎는 순간에만 맞은 것으로 친다.
        // ★★ **폰은 데스크탑보다 여유 있게 잡는다**(2026-08-04, 쓰는 사람이 "너무 어렵다").
        //   손가락은 마우스처럼 정확히 못 찍고, 배경화면은 아이콘 사이 빈 곳만 눌리며,
        //   누르는 순간이 곧 판정이라 되돌릴 틈도 없다. 창을 **4프레임(133ms) → 6프레임
        //   (200ms)** 으로 넓히고, **한 번 놓쳐도 이어지게**(`BEAT_WINDOW` 40 > 두 번째
        //   기회까지의 19프레임) 뒀다. 그래도 **아무 때나 치면 콤보는 0으로 끊긴다** —
        //   박자를 맞혀야 하는 놀이라는 것 자체는 그대로다.
        const val BEAT_AT = 1.1f         // 제자리보다 이만큼 아래(칸)까지 내려가야
        const val BEAT_V = 1.35f         // 그 자리에서 이만큼 느려야 (되돌아 오르는 꼭짓점)
        const val BEAT_WINDOW = 40       // 이 프레임 안에 다시 맞혀야 이어진다 (1.3초)
