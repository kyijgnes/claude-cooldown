...
- 슬림 바: 왼쪽 끝 **상태 띠(`accent`, 한도 색 초록/노랑/빨강)** + **통통 튀는 클로드 별빛 마스코트 '클로디'**
  ...
  - **생김새는 폰 앱 아이콘(`ic_claudi.xml`)과 같은 결이다** — 상하좌우 **네 살**만 뻗고
    대각선 넷은 **떠 있는 반짝임 점**(`_rays`), 얼굴은 큰 눈 둘 + **웃는 입**(`_mouth`).
    옛날엔 여덟 살을 길고 짧게 번갈아 뻗었는데, 이 크기에서 눈에 들어오는 건 긴 넷뿐이라
    짧은 살은 팔다리처럼만 보였다.
  - ★ **눈은 반짝임 점보다 확실히 커야 한다**(눈 r=1.5 / 점 r=0.95). 비슷하면 20px 에서
    둘 다 `+` 로 뭉개져 **얼굴이 아니라 반짝임이 셋 있는 것처럼** 보인다.
