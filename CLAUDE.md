...
- **실패·놓친 앵커 표시** — 컴퓨터가 꺼져 있던 등으로 앱이 안 떠 있는 동안 앵커의 여유(grace)
  ...
  `HH:MM 자동 시작 놓침` / `HH:MM 자동 시작 실패`, 트레이 알림·툴팁에도 남긴다.
...
- 슬림 바: 왼쪽 끝 **상태 띠(`accent`, 한도 색 초록/노랑/빨강)** + **통통 튀는 도트 마스코트 '클로디'**
  ...
  - **오른쪽 윗자리는 '번갈아 칸'이다**(`SLOT_*` 상수 · `_slot_*` 메서드). 한 줄뿐인데 보여 줄 것이
    여럿이라 `SLOT_HOLD`(4.2초)마다 하나씩 바뀐다 — 보여 주던 것이 위로 밀리며 바탕색에 스미고
    (`_mix`. tk 캔버스 글자엔 투명도가 없어 색을 바탕 쪽으로 당겨 흉내 낸다) 다음 것이 아래에서
    떠오른다. 도는 것: **모델별 한도(`Fable 7%`) → 주간 적정선(`적정선 62%`) → 알림**.
    ★ **적정선을 넣은 까닭이 둘이다** — 게이지의 ┃ 눈금이 몇 %인지 슬림 바에서는 볼 데가
    없었고, 늘 두 가지 이상이 돌아야 **알림이 없을 때 그 자리가 비어 보이지 않는다**
    (자리 폭은 가장 긴 알림 문구에 맞춰 잡혀 있다 — 아래 `width` 참고).
    ...
    - 오류(`show_error`)는 이 자리를 통째로 쓰므로 도는 것을 세운다(`_slot_on=False`).
      애니메이션은 마스코트처럼 **세대 토큰(`_slot_gen`)** 으로 옛 루프를 은퇴시킨다.
    - ★ 밀리는 거리(`SLOT_RISE`)는 **5px 를 넘기지 말 것** — 더 크면 아래 게이지를 침범한다
      (작업표시줄 48px 기준 글자 아래 여유가 8px 뿐이다).
