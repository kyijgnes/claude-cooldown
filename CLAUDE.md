...
- `cooldown_core.py`·`cooldown_ping.py` 가 저장소 루트에 있어 `--paths` 로 루트와
  `windows` 를 둘 다 준다(+ `HIDDEN` 에 명시). 루트 모듈을 새로 추가하면 `HIDDEN` 에도 넣을 것.
...
1. **라이브 배경화면을 '상어' 테마로 바꾸기** ← 다음에 바로 할 것.
   사용자가 쓰던 상어 배경화면의 결을 가져온다. 실측 팔레트:
   바탕 `#bbd8f3` · 상어 몸통 `#9bc3ea` · 배 `#ffffff` · 윤곽 `#0d1115` ·
   물방울 `#cce2f6` · 그림자 `#a2c6e3`. 고칠 곳은 `wallpaper/WallpaperArt.kt` 한 곳.
   게이지는 지금처럼 아래 1/3 유지(잠금화면 시계가 위를 쓴다).
   `build.ps1 test` 로 `app/build/미리보기/배경화면.png` 를 보며 다듬는다.
