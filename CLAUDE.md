...
| `android/art/paint_gums.py` | 상어 목구멍에 분홍 한 점 넣는 아트 도구 (+ 칠하기 전 원본 보관) |
...
- **상어는 손그림이 아니라 그림 파일이다** — `drawable-nodpi/shark.png`(입 다문, 기본) ·
  `shark_open.png`(입 벌린). 폰에 깔려 있던 갤럭시 테마 "BabyShark" 에서 뽑은 원본이고,
...
- ★ **잠금화면 상어는 입을 다문 쪽이다** — 폰 원본이 그랬다(디자인 프로젝트 README 에는
  "입 벌린"으로 적혀 있는데 그게 틀렸다). 잠금을 풀면 입 벌린 쪽으로 바뀐다
  (아래 '상어 얼굴은 잠금 상태가 정한다').
- ★ **핑크는 '아주 조금'이다 — 입꼬리 안쪽 목구멍 한 곳뿐**(`android/art/paint_gums.py`).
  원본 배경화면(`homescreen_wallpaper.png`, 1920²)을 픽셀로 훑으면 **분홍이 그림 전체에서
  58px**, 그것도 위·아래 턱이 만나는 **주머니 하나**다. 색은 `#FFC0D5`(실측).
  · 그래서 **잉크로 완전히 둘러싸인 그 주머니만 물 붓듯 채운다**(`SEED` 에서 flood fill).
    새어 나가면(`MAX_FILL` 초과) **저장하지 않고 실패로 끝낸다.**
  · **입 다문 쪽은 안 칠한다** — 원본 스프라이트에도 분홍이 없다.
  · ✗ 해 보고 되돌린 것들: 입 안 통째로 / 이빨 따라 띠 두르기 / 입선 둘레 흰 띠.
    전부 원본과 딴판이 된다. **"잇몸이 약간 핑크"의 '약간'이 핵심이다.**
  · 원본은 `art/원본_칠하기전/` 에 보관하고 스크립트는 **늘 거기서 시작**한다(여러 번 돌려도
    같은 결과). ★ 이 폴더에 **칠한 그림을 넣지 말 것** — 한 번 섞이면 돌릴 때마다 옛 핑크가
    되살아난다. 실제로 오염돼서 git 이력(`2310b56`)에서 되꺼냈다.
- ★★ **눈으로 짐작하지 말고 원본을 픽셀로 볼 것.** 테마 APK 가 폰에 아직 깔려 있다:
  `homescreen_wallpaper.png`(입 벌린 상어 + 분홍 목구멍) · `component_2.png`(헤엄치는 상어,
  330×188, 입 다뭄, 분홍 없음). **`adb pull` 은 권한으로 막힌다 — `adb exec-out cat`**
  (`paint_gums.py` 머리말에 명령 그대로 있다). 이걸 안 하고 그림만 보고 짐작하다가
  **세 번 연달아 엉뚱한 데를 칠했다.**
...
배경은 **내 사진(또는 떠 온 '쓰던 배경') / 상어** 둘 중 하나다. 미터기 모양(막대 / 링 /
숫자만 / 없음)을 고르고 **자리는 미리보기를 손가락으로 끌어서** 정한다.
옵션 > `배경화면 꾸미기`. (UI 어휘는 '상어' — '상어 바다'라고 쓰지 않는다)
...
- **폰에 걸려 있던 배경화면 떠 두기**(`WallpaperGrab`, `filesDir/phone_wallpaper.png`) —
  **우리 라이브 배경화면이 걸리기 전에만 뜰 수 있다**(걸린 뒤엔 '지금 배경화면'이 우리다).
  우리 게 안 걸려 있으면 **앱을 열 때마다 다시 뜬다** — 사용자가 배경을 바꾸면 따라가야 하니까.
...
- ★ **'쓰던 배경'이라는 칸 이름은 떠 온 그림이 실제로 있을 때만 쓴다.** 없는데 그렇게 적으면
  화면엔 상어가 뜨는데 이름만 '쓰던 배경'이라 거짓말이 된다 — 그 경우 이름은 `내 사진`이고,
  고른 사진도 떠 온 그림도 없으면 칸을 누를 때 **사진 고르는 화면부터** 띄운다.
  직접 고른 사진을 쓰는 중이면 `쓰던 배경으로` 버튼이 하나 더 선다(떠 온 그림이 있을 때만).
- **설명 대신 미리보기다.** 고르면 그 자리에서 바뀌고, 크기 % 는 칸 이름에 그대로 적는다
  (`상어 크기 · 120%`). 툴팁·도움말 문단은 넣지 않는다(폴더 전체 규칙).
...
- ★ **상어 자리는 상어 자신의 한가운데**다(물방울 판의 한가운데가 아니라). 판 기준으로
  잡으면 크게 키웠을 때 상어만 화면 밖으로 밀려난다. 사진 자리(`photoX/Y`)는 **따로 담는다** —
  사진을 옮겨 놓고 상어 바다로 돌아왔을 때 상어까지 따라 움직이면 안 된다.
- **사진 배경**: `ACTION_OPEN_DOCUMENT` + `takePersistableUriPermission`(안 잡으면 재부팅 후
  사진이 사라진다). 읽기는 `WallpaperArt` 가 **주소+화면크기 키로 캐시**한다 — 16fps 로
  매 프레임 디코딩하면 폰이 뜨거워진다. 폰 사진은 4000×3000(48MB)이 흔해서
  **화면 넓이의 3배로 줄여 읽는다.** 못 읽으면(안 골랐거나 지웠거나) **빈 파랑이 아니라
  상어 바다로 내려간다.**
...
**잠겨 있으면 입을 다물고, 풀면 벌린다.** 고르는 항목이 아니라 저절로 그렇게 된다 —
설명할 것도 없고, 잠금을 풀 때마다 한 번씩 재미있다. 판정은
`KeyguardManager.isKeyguardLocked` 이고 `WallpaperArt.render(..., locked)` 로 넘어간다.
바뀌는 순간 `notifyColorsChanged()` 도 부른다(아래).
...
2. ~~배경화면 실기 확인~~ **완료 (2026-07-31, SM-G988N 무선 디버깅).** 확인한 것:
   라이브 배경화면이 **홈·잠금 양쪽에 걸리고**(잠금화면 시계와 안 겹침), 입 다문 상어와
   핑크 잇몸이 실제 크기에서 의도대로 보이고, 꾸미기 화면의 **끌기가 손가락을 정확히 따라오고**
   (미터기 −80/+130px 이동, 상어는 그대로 → 반대로 배경을 끌면 상어만 −75/+130px 이동),
   `내 사진` 을 고르면 문서 선택기가 열리고 상어 칸이 사라지며 **`글씨 뒤 판`이 자동으로 켜지고**,
   `처음 모양으로` 가 전부 되돌린다.
   **아직 안 본 것**: 사진을 실제로 골라 배경으로 깐 모습, 링·숫자만 미터기의 실기 모습
   (렌더 그림으로만 봤다), 홈 위젯, 상태바 글씨(`windowLightStatusBar`).
   디자인 프로젝트 `theme_package/README.md` 는 아직 "배경화면 = 입 벌린"이라고 적혀 있다 —
   실제로는 입 다문 쪽이므로 그 문서를 손볼 때 같이 고칠 것.
   > 무선 디버깅은 **페어링 포트와 연결 포트가 다르다.** 팝업(`기기 페어링`)의 번호로
   > `adb pair` 를 하고, 본문 `IP 주소 및 포트` 의 번호로 `adb connect` 를 한 번 더 해야 한다.
   > 회사망(172.16/16)에서는 mDNS 자동 검색이 안 잡혀 주소를 손으로 넣어야 한다.
