...
**기본은 '쓰던 배경화면 + 미터기'다.** 상어 바다는 골라서 쓴다. 미터기 모양(막대 / 링 /
...
- ★ **처음 켤 때 폰에 걸려 있던 배경화면을 한 장 떠 둔다**(`WallpaperGrab`, `filesDir/phone_wallpaper.png`).
  **우리 라이브 배경화면이 걸리기 전에 떠야 한다** — 걸린 뒤엔 '지금 배경화면'이 우리다.
  그래서 `wallpaperInfo?.packageName == 우리` 면 안 뜨고, 파일이 이미 있으면 덮어쓰지 않는다.
  못 떠 와도(권한·기기 제약) 그만이다 — 상어 바다로 내려간다.
...
데스크탑 위젯 슬림 바의 코랄 별빛 마스코트를 폰으로 옮겼다 — PC·폰이 한 앱으로 읽힌다.
(상어는 배경화면 쪽에 그대로 있다)

> ★ **2026-08-02 부터 데스크탑 클로디는 별빛이 아니라 도트 캐릭터다**(위 '작업표시줄에 붙이기' 절).
> 폰 아이콘·상태바 반짝임은 **아직 옛 별빛 그대로**라 둘이 다르다. 폰 쪽을 맞출 때는
> 이 절의 '★' 들(구멍으로 뚫기 등)을 그대로 지키면서 도트 그림으로 갈아끼우면 된다.

- ★ **데스크탑 비율을 그대로 키우면 '톱니바퀴'가 된다.** 거기는 지름 20px 라 몸통이 크고
  살이 짧아도 반짝임으로 읽히지만, 아이콘 크기에선 아니다 → 몸통을 줄이고(0.62R→0.42R)
  살을 길고 가늘게 뺐다. **대각선 넷은 아예 떠 있는 점**으로 바꿨다 — 짧은 선으로 두면
  팔다리처럼 보이고, 어차피 눈에 들어오는 건 상하좌우 네 살뿐이다.
- ★ **눈·입은 흰 도형이 아니라 `evenOdd` 로 뚫은 구멍**이다. 그래야 단색(monochrome)
  아이콘에서도 얼굴이 남는다(흰색으로 칠하면 통째로 물들어 사라진다).
- ★ **살은 몸통 가장자리에서 시작한다.** 가운데에서 그으면 몸통 밑을 가로지르는데,
  눈이 구멍이라 **그 살이 눈으로 비쳐 나와 실눈처럼 뭉개진다.**
...
- **숫자를 클로디 반짝임이 감싼다** — 앱 아이콘과 같은 결이라 상태바에서도 우리 것인 줄 안다.
  ★ **네 모서리에 점으로만** 놓는다. 처음에 대각선 짧은 '살'로 그렸더니 24dp 에서
  숫자 획과 섞여 `17` 이 `)17(` 처럼 읽혔다 — 실기에서 확인하고 점으로 바꿨다.
