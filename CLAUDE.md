...
- ★ **잇몸은 두 장 다 칠해야 한다** — `android/art/paint_gums.py`(원본은 `art/원본_흰잇몸/` 에
  보관, 여러 번 돌려도 같은 결과). 입 다문 쪽만 칠하면 **잠금을 푸는 순간 핑크가 사라져서
  "반영이 안 됐다"고 느낀다** — 실제로 그렇게 헤맸다.
  · 입 다문 쪽: 입선 위아래의 흰 띠
  · 입 벌린 쪽: **입안을 통째로** (폰 테마의 정면 상어 아이콘이 딱 이 모습이다).
    입이 왼쪽으로 열려 있어 통째로 칠하면 얼굴 밖으로 삐져나올 것 같지만, **위·아래 이빨이
    둘 다 있는 세로줄만** 칠하므로 입꼬리 오른쪽·주둥이 끝 왼쪽으로는 저절로 안 넘어간다.
