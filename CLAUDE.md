...
- ★ **배경화면 상어는 입을 다문 쪽이다** — 폰 원본이 그랬다(디자인 프로젝트 README 에는
  "입 벌린"으로 적혀 있는데 그게 틀렸다). **잇몸도 살짝 핑크**라, 받은 아트의 흰 입선 둘레를
  `android/art/paint_gums.py` 로 한 번 칠해 넣었다(원본은 `art/shark_원본_흰잇몸.png` 에 보관).
  입 벌린 얼굴은 꾸미기에서 고를 수 있다.
...
배경(상어 바다 / 바다색만 / 내 사진)과 미터기(막대 / 링 / 숫자만 / 없음)를 고르고,
**자리는 미리보기를 손가락으로 끌어서** 정한다. 앱 화면 > `배경화면 꾸미기`.

- **값은 `Look` 한 곳**에 있다(`Look.kt`). `WallpaperArt` 는 그리기만, `CustomizeActivity` 는
  고르기만 한다. 새 꾸미기 항목을 더할 때도 값은 반드시 `Look` 에 둔다.
- **기본값은 꾸미기를 넣기 전 화면 그대로**다(1080×2340 에서 잰 값). 처음 켠 사람은
  달라진 걸 못 느끼고, 만지고 싶은 사람만 만진다. `Look.reset` 이 이 값으로 되돌린다.
...
- **끌면 짚은 것이 따라온다** — 미터기를 짚으면 미터기가(`hitsMeter`), 그 밖을 짚으면 배경이.
  끌기 계산은 `WallpaperArt.dragMeter`/`dragArt` 안에만 둔다(화면 쪽은 픽셀 차이만 넘긴다).
...
- ★ **배경 자리는 상어 자신의 한가운데**다(물방울 판의 한가운데가 아니라). 판 기준으로
  잡으면 크게 키웠을 때 상어만 화면 밖으로 밀려난다.
- **사진 배경**: `ACTION_OPEN_DOCUMENT` + `takePersistableUriPermission`(안 잡으면 재부팅 후
  ...
  **화면 넓이의 3배로 줄여 읽는다.** 못 읽으면 조용히 바다로 내려간다.
- 사진 위에서는 글씨가 안 보이므로 **`글씨 뒤 판`이 사진일 때 기본으로 켜진다**
  (`Look.plate` 의 '안 정했으면' 규칙). 사용자가 끄면 그 선택이 이긴다.
...
2. **배경화면·홈 위젯 실기 확인** — 폰 잠금을 풀어야 미리보기를 띄울 수 있어 아직 못 봤다.
   ...
   **꾸미기 화면의 끌기·사진 고르기도 실기 확인이 남았다**(렌더 그림으로만 봤다).
