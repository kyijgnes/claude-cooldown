...
- **상어는 손그림이 아니라 그림 파일이다** — `drawable-nodpi/shark_open.png`(배경화면) ·
  `shark.png`(앱 아이콘). 폰에 깔려 있던 갤럭시 테마 "BabyShark" 에서 뽑은 원본이고,
  배치·박자(883ms 왕복, 물방울 1440/1600/1920ms)도 그 테마의 `animation.xml` 실측치다.
  출처·팔레트·좌표는 클로드 디자인 프로젝트 **`Shark Wallpaper Request`**
  (`<디자인-프로젝트-id>`) 의 `theme_package/README.md` — DesignSync 로 읽는다.
  **어둡게에서는 그림을 못 바꾸므로 `shark_tint` 로 곱해서 내린다.**
...
1. ~~라이브 배경화면을 '상어' 테마로~~ **완료.** 앱 전체가 파란 '상어' 테마다:
   `colors.xml`(밝게/어둡게) 의 `bg`·`track`·`line` 이 파랑이라 메인화면·위젯도 파랑이고,
   `WallpaperArt.kt` 는 **폰 테마에서 뽑은 상어 그림**을 원본과 같은 자리·같은 박자로 띄운다
   (위 '폰 앱' 절 참고). 앱 아이콘도 폰 아이콘팩과 같은 결로 맞췄다 — 흰 바탕 + 그 상어 그림.
   **게이지 초록/노랑/빨강은 의미(여유·보통·임박)라 그대로 뒀다** — 파랑은 배경·아이콘에서만.
   게이지는 아래 1/3 유지. 확인은 `build.ps1 test` → `app/build/미리보기/`
   (`배경화면` · `배경화면_내려감` · `배경화면_어둡게` · `앱아이콘_432`).
2. **배경화면·홈 위젯 실기 확인** — 폰 잠금을 풀어야 미리보기를 띄울 수 있어 아직 못 봤다.
   상태바 글씨 수정(`windowLightStatusBar`)도 설치만 하고 눈으로 확인은 못 했다.
3. **작업표시줄용 슬림 바 다듬기** — 크기는 맞았고 보기 좋게가 남았다.
   요청서는 `docs/디자인_요청서.md`
4. `limits[]` 의 `severity` 를 색상 임계값 대신 쓸지 검토 (지금은 50/80 자체 기준)
5. 폰 마스코트를 데스크탑 '클로디' 수준으로 (지금은 숨쉬기·통통·깜빡만)
