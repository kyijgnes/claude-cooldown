...
1. ~~라이브 배경화면을 '상어' 테마로~~ **완료.** 앱 전체를 파란 '상어' 테마로 바꿨다:
   `colors.xml`(밝게/어둡게) 의 `bg`·`track`·`line` 을 파랑으로 틴트해 메인화면·위젯이
   자동으로 파랑이 되고, `WallpaperArt.kt` 는 바닷속 그라데이션 + 헤엄치는 파란 상어(꼬리 젓기·
   숨쉬기) + 물방울로 다시 그렸다. 앱 아이콘도 바다색 + 흰 상어 실루엣(`ic_launcher_bg/fg`).
   상어 팔레트는 `colors.xml` 의 `sea_*`·`shark_*`·`bubble`(실측: 바탕 `#bbd8f3`·몸통 `#9bc3ea`·
   배 `#fff`·윤곽 `#0d1115`·물방울 `#cce2f6`·그림자 `#a2c6e3`). **게이지 초록/노랑/빨강은
   의미(여유·보통·임박)라 그대로 뒀다** — 파랑은 배경·마스코트·아이콘에서만.
   게이지는 아래 1/3 유지. 확인은 `build.ps1 test` → `app/build/미리보기/배경화면.png`.
