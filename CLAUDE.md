...
- ★ **잇몸(핑크)은 입 벌린 쪽에만 넣는다** — `android/art/paint_gums.py`.
  **폰 테마 원본 스프라이트에는 핑크가 아예 없다**(아래 '원본 확인'). 다문 입은 선 하나라
  잇몸이 보일 자리가 없다. 핑크는 폰 테마의 **정면 상어 아이콘** 색(`#FFC0D5` 실측)이고,
  옆모습에서는 벌린 입 안이 그 자리다.
  · 칠하는 규칙은 **두 걸음**이다. ① 위·아래 이빨이 **둘 다 있는 세로줄**에서만 그 사이의
    빈 곳(입 안)을 찾고 ② 그중 **이빨에 닿아 있는 테두리만** 남긴다.
    ①만 하면 입 안이 통째로 분홍이 된다 — 그건 **잇몸이 아니라 '입이 핑크'** 다.
    ①에서 세로줄 조건을 빼면 아래턱이 없는 칸(주둥이 앞)까지 새어 **핑크 덩어리가
    입 밖 바다에 떠 있게 된다.** 둘 다 실제로 겪고 되돌린 것이니 순서를 지킬 것.
...
- ★ **원본 확인 — 테마 APK 가 폰에 아직 깔려 있다.** 의심스러우면 여기서 직접 본다:
  `res/drawable-xxhdpi-v4/component_2.png` (330×188, 입 다뭄, 핑크 없음).
  **`adb pull` 은 권한으로 막힌다 — `adb exec-out cat` 을 쓸 것** (`paint_gums.py` 머리말에 명령).
