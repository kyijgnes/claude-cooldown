...
- **놓친 앵커 표시** — 컴퓨터가 꺼져 있던 등으로 앱이 안 떠 있는 동안 앵커의 여유(grace) 구간이
  다 지나가면 그 앵커는 핑도 못 쏘고 처리 기록도 못 남긴다 = **놓침**. 위젯 게이지 근처(스킨별
  꼬리말·제목·모델별 자리)에 호박색으로 `HH:MM 자동 시작 놓침`, 트레이 알림·툴팁에도 남긴다.
...
- ★ **`abort()` 로 죽으면 우리 로그에는 한 글자도 안 남는다.** `Py_FatalError` → `abort()` 는
  윈도우에서 `__fastfail` 로 곧장 끝나 `faulthandler` 의 SIGABRT 처리가 안 돈다. 그래서
  **로그에 `시작` 만 있고 그 뒤가 통째로 빈 것이 곧 이 크래시의 모습**이다 — 밖에서 죽인
  것과 구별이 안 되니 **이벤트 뷰어부터 볼 것**(`예외 코드 0xc0000409`, 모듈 `ucrtbase.dll`).
- 덤프(`%LOCALAPPDATA%\CrashDumps\*.dmp`)는 디버거 없이도 읽을 수 있다 — 미니덤프의
  모듈 목록과 예외 스레드 스택을 훑어 **주소를 가장 가까운 내보낸 함수 이름으로** 바꾸면
  어느 계층에서 죽었는지 나온다. 2026-08-03 크래시는 이렇게 잡았다:
  `abort` ← `_Py_FatalErrorFunc("PyEval_RestoreThread")` ← Tcl 타이머 → Tk 창 프로시저
  ← `NtUserSetWindowPos` ← `win32gui.pyd`. (아래 '하지 말 것' 의 GIL 규칙)
...
- 오류 문구는 `cooldown_core` 가 **짧은 명사형**으로 던진다. 슬림 바의 오른쪽 자리
  (`right_w`)가 **104px** 뿐이라 길면 잘린다. 새 문구를 만들 때 폭을 재 볼 것.
  ★ **그 자리에 새 문구를 넣으면 `slim.py` 의 `MAX_*` 표본에도 반드시 넣는다** —
  폭은 그 표본들의 최댓값으로 잡히므로, 빠뜨리면 화면에서만 조용히 잘린다.
  실제로 `notice()`(자동 시작 놓침)가 표본에서 빠져 104px 짜리가 64px 자리에 들어갔었다.
  `width` 를 줄이면 두 한도 칸이 먼저 제 몫(324px)을 가져가고 **오른쪽 자리부터 깎인다**.
...
- **작업표시줄도 '항상 위' 라서 그냥 topmost 로 두면 가려진다.** 조작할 때마다
  스스로를 올리므로 `raise_above_taskbar()`(= `root.lift()`)를 `STAY_TICK`(250ms)마다
  다시 부른다. **win32 로 직접 올리지 말 것** — 아래 '하지 말 것' 의 GIL 규칙.
...
- ★★ **창을 만지는 Win32 함수를 `win32gui`·`ctypes.windll` 로 부르지 말 것**
  (`SetWindowPos`·`EndMenu`·`DwmSetWindowAttribute`·`ShowWindow`…).
  둘 다 **호출하는 동안 GIL 을 놓는데**, 그 함수들은 부르는 그 자리에서 창 프로시저로
  메시지를 보내고 **Tk 는 거기서 밀린 `after` 콜백을 실행한다** — 파이썬 스레드 상태가
  없는 채로 들어가 `Fatal Python error: PyEval_RestoreThread` 로 **앱이 통째로 죽는다.**
  마스코트 애니메이션이 45ms 마다 도니까 밀린 콜백은 사실상 늘 있다.
  · 올리기는 **`root.lift()`**(Tk 의 raise). Tcl 안에서 같은 `SetWindowPos` 를 부르므로
    스레드 상태가 멀쩡하다. 작업표시줄을 이기는 효과는 win32 방식과 같은 것으로 확인했다.
  · Tk 로 못 하는 것은 **`gil_held("user32")`** 처럼 `ctypes.PyDLL` 로 부른다(GIL 을 안 놓는다).
  · 읽기만 하는 것(`FindWindow`·`GetWindowRect`·`IsWindowVisible`)은 메시지를 안 보내니
    지금처럼 `win32gui` 그대로 써도 된다.
  2026-08-03 02:43 에 실제로 이걸로 죽었다 — 위젯을 떼어 놨다가 디자인에서 슬림 바를
  다시 골라 작업표시줄에 되붙는 순간(`show_window` → `raise_above_taskbar`)이었다.
